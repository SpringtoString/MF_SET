...
if __name__ == '__main__':
    ...
    alpha = 20
    C[user_train_idx, item_train_idx] = alpha * ratting_train
    ...
    R_test = torch.tensor(R_test, dtype=torch.double)




    model=LMF(n_users, n_items, n_factor=10)
    learning_rate=0.01
    optimizer=torch.optim.Adam(params = model.parameters() , lr = learning_rate)
    for epoch in range(200):
        optimizer.zero_grad()
        A=model.forward()
        loss = C * A - (1 + C) * (torch.log((1 + torch.exp(A))))
        loss = -loss.mean()
        loss.backward()
        optimizer.step()
        if (epoch % 10 == 0):
            with torch.no_grad():
                print(epoch, "mpr is ",MPR(A,R_test))

    factor_list = [10, 20, 40]
    for f in factor_list:
        print(("f=",f))
        model = LMF(n_users, n_items, n_factor=f)
        learning_rate = 0.01
        optimizer = torch.optim.Adam(params=model.parameters(), lr=learning_rate )
        for epoch in range(200):
            optimizer.zero_grad()
