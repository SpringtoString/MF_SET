...
class PMF(nn.Module):
    def __init__(self, n_user, n_item, n_factor, I):
        '''
        :param C:
        :param R_test:
        :param n_factor:
        ...
        self.W = Parameter(torch.randn([self.n_item, self.n_factor], dtype=torch.double))

    def forward(self):
        A =self.I.mm(self.W)/(self.I.sum(1).view(n_users , -1))
        A = self.X+A
        A = torch.mm(A, self.Y.T)
        return A
...
if __name__ == '__main__':
    ...
    model = PMF(n_users, n_items, 10,I)
    learning_rate = 0.01
